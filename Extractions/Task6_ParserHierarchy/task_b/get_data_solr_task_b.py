...
def get_solr_response():
	...
	for i in range(0,30):
		try:
			url="http://localhost:8983/solr/language/select?q=*%3A*&fl=metadata_length,size,Content-Type,X-Parsed-By&wt=json&indent=true&start="+str(start)+"&rows="+str(rows)
	...
	for entry in data["response"]["docs"]:
		try:
			...
			if content_type not in parser_dict[val]:
				...
				parser_dict[val][content_type]["metadata_length"]=0
			parser_dict[val][content_type]["size"]+=entry["size"]
			parser_dict[val][content_type]["metadata_length"]+=entry["metadata_length"]
			count+=1
		except KeyError:
			pass
